    ...
    <section id="capabilities" className="pb-20 md:pb-28">
      <div className="wrap">
        <Reveal className="mb-10 max-w-xl border-b border-line pb-5">
          <p className="eyebrow">
            <GlitchText text="FIG. 03 — Instruments" interval={2500} />
          </p>
          <h2 className="display mt-2 text-4xl md:text-5xl">
            <GlitchText text="Build. Verify. Secure." interval={1800} />
          </h2>
        </Reveal>
        ...
        <div className="grid gap-8 border-b border-line pb-10 md:grid-cols-3 md:gap-10">
          {pillars.map((p, i) => (
            <Reveal key={p.title} delay={i * 0.04}>
              <div className="glass fx-border-pulse h-full p-5 md:p-6">
                <p className="mono text-[0.62rem] text-accent">0{i + 1}</p>
                <h3 className="display mt-3 text-2xl md:text-3xl">
                  <GlitchText text={p.title} interval={1700 + i * 200} />
                </h3>
        ...
        <Reveal className="pt-10">
          <p className="mono text-[0.65rem] text-mute">
            <GlitchText text="TOOLKIT" interval={3000} />
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            {skills.map((skill, i) => (
              <span key={skill} className="chip">
                <GlitchText text={skill} interval={2200 + (i % 5) * 300} live={i % 3 === 0} />
              </span>
            ))}
          </div>
        </Reveal>
